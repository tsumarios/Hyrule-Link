// main.js
if (!window.crypto || !window.crypto.getRandomValues) { window.crypto = { getRandomValues: arr => arr.map(() => Math.floor(Math.random() * 256)) }; }
...
// --- CRYPTO ---
function hexToUint8Array(hex) { const arr = new Uint8Array(hex.length / 2); for (let i = 0; i < arr.length; i++)arr[i] = parseInt(hex.substr(i * 2, 2), 16); return arr; }
function generateKey() { const k = nacl.randomBytes(32); return Array.from(k).map(b => b.toString(16).padStart(2, '0')).join(''); }
function getHash(k) { const b = hexToUint8Array(k); const h = nacl.hash(b); return Array.from(h.slice(0, 32)).map(b => b.toString(16).padStart(2, '0')).join(''); }
...
async function init() {
    ...
    let clean = choice.toLowerCase().replace(/[^0-9a-f]/g, '');
    if (clean.length !== 64) { alert("❌ Key must be 64 characters."); return init(); }
    setupSession(clean);
}
...
function setupSession(k) {
    ...
    keyBox.onclick = () => {
        const ta = document.createElement("textarea"); ta.value = k; document.body.appendChild(ta); ta.select();
...
socket.on('nudge_alert', sender => {
    ...
    socket.on('history', hist => {
        hist.forEach(m => processIncoming(m));
    });
    ...
    if(type === 'theirs' && senderName) {
            const span = document.createElement('span'); span.className='sender-name'; span.textContent=senderName; div.appendChild(span);
        }
    ...
    const bar = document.createElement('div'); bar.className='reactions';
        ['❤️','😂','⚔️','🛡️'].forEach(emoji => {
            const btn = document.createElement('span'); btn.className='rxn-btn';
    ...
    function addSystemMsg(text){
        const div = document.createElement('div'); div.className = 'system-alert'; div.textContent = text;
    messages.appendChild(div); messages.scrollTop = messages.scrollHeight;
    }
